...
/**
 * @api {get} /exposures        Query ODI exposures
 ...
 */
router.get('/exposures', jwt({secret: config.odi.auth_pubkey}), function(req, res, next) {
    var find = {};
    ...
    .select(req.query.select || 'logical_id')
    .limit(req.query.limit || 100)
    .skip(req.query.skip || 0)
    .sort(req.query.sort || '_id')
    .exec(function(err, exposures) {
        if(err) return next(err);
        //console.dir(exposures);
        db.Exposures.count(find).exec(function(err, count) {
            if(err) return next(err);
            res.json({exposures: exposures, count: count});
        });
    });
});
